using log4net;
using log4net.Config;
using System.Reflection;
using System.Xml;

namespace CRMApi.Helpers.Logger
{
    public class LoggerManager : ILoggerManager
    {
        /// <summary>
        /// Class that will be creating the row log that happened during the process
        /// </summary>
        private readonly ILog _logger = LogManager.GetLogger(typeof(LoggerManager));

        public LoggerManager()
        {
            try
            {
                XmlDocument log4netConfig = new();

                using var fs = File.OpenRead("log4net.config");
                log4netConfig.Load(fs);

                var repo = LogManager.CreateRepository(
                        Assembly.GetEntryAssembly(),
                        typeof(log4net.Repository.Hierarchy.Hierarchy));
                string route = Path.GetDirectoryName(Assembly.GetExecutingAssembly().GetName().CodeBase) ?? string.Empty;
                GlobalContext.Properties["FilePath"] = route[6..];
                XmlConfigurator.Configure(repo, log4netConfig["log4net"]);
            }
            catch (Exception)
            {
                throw;
            }
        }

        public void LogInformation(string message)
        {
            _logger.Info($"[INFO]: {message}");
        }

        public void LogInformation(string message, Exception ex)
        {
            _logger.Info($"[INFO]: {message}", ex);
        }

        public void LogWarning(string message)
        {
            _logger.Warn($"[WARNING]: {message}");
        }

        public void LogWarning(string message, Exception ex)
        {
            _logger.Warn($"[WARNING]: {message}", ex);
        }

        public void LogError(string message)
        {
            _logger.Error($"[ERROR]: {message}");
        }

        public void LogError(string message, Exception ex)
        {
            _logger.Error($"[ERROR]: {message}", ex);
        }
    }
}
